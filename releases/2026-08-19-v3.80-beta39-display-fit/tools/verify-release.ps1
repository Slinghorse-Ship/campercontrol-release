...
foreach ($deployScript in @("tools/deploy-gx.sh", "tools/deploy-wasm.sh")) {
...
try {
    ...
    if (-not $transportFacade.Contains('Qt.platform.os === "wasm" ? mqttTransport : httpTransport')) {
        throw "Transport facade does not select MQTT for WASM"
    }
