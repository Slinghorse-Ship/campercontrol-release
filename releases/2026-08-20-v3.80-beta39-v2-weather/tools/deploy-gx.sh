...
# Atomic native GX deployment for the frozen V2-only weather build.
# The old active tree is first renamed to one exact gui-v2.pre-* path. That
# exact tree is compressed and SHA-256 verified on /data before it is removed
# from the small root filesystem.
set -eu
...
artifact_source_commit=ee6a8f8897832d3e63ffbe98ffdd4389708dfc1e
expected_source_commit=ee6a8f8897832d3e63ffbe98ffdd4389708dfc1e
...
stage=/data/campercontrol/staging/camper-gui-v2-gx-v2-weather-20260820
active=/opt/victronenergy/gui-v2
candidate=/opt/victronenergy/gui-v2.candidate-v2-weather-20260820
pre_tree=/opt/victronenergy/gui-v2.pre-v2-weather-20260820
failed_tree=/opt/victronenergy/gui-v2.failed-v2-weather-20260820
...
expected_hash=2e216949aeac78c7ab1151cce994caa19b7e37afeee098ec9273750a272df7d6
expected_files=923
...
archive_and_remove_exact_pre_tree() {
	...
	test "$archived_files" = "$pre_files"
	# Only this literal, validated pre-tree is removed. There is no wildcard or
...
test -f "$stage/Victron/VenusOS/pages/camper/CamperShell.qml"
test -f "$stage/Victron/VenusOS/pages/camper/v2/CamperV2Shell.qml"
test -f "$stage/Victron/VenusOS/pages/camper/v2/CamperV2QuickPanel.qml"
test -f "$stage/Victron/VenusOS/pages/camper/v2/CamperV2WeatherPanel.qml"
test -f "$stage/Victron/VenusOS/pages/camper/v2/CamperV2PanelHost.qml"
test -f "$stage/Victron/VenusOS/data/camper/CamperWeatherAdapter.qml"
test -f "$stage/Victron/VenusOS/components/camper/v2/CamperV2WeatherChart.qml"
test -f "$stage/Victron/VenusOS/components/camper/v2/CamperV2Header.qml"
for forbidden in \
	"$stage/Victron/VenusOS/pages/camper/CamperHome.qml" \
	"$stage/Victron/VenusOS/pages/camper/CamperLights.qml" \
	"$stage/Victron/VenusOS/pages/camper/CamperPower.qml" \
	"$stage/Victron/VenusOS/pages/camper/CamperDetails.qml" \
	"$stage/Victron/VenusOS/components/camper/CamperDesignSettings.qml"
do
	test ! -e "$forbidden"
done
grep -q 'CamperV2Shell' "$stage/Victron/VenusOS/pages/camper/CamperShell.qml"
if grep -Eq 'designVersion|CamperDesignSettings|CamperHome' "$stage/Victron/VenusOS/pages/camper/CamperShell.qml"; then
	printf '%s\n' 'V2_ONLY_CONTRACT_FAILED: legacy design selector/reference found' >&2
	exit 6
fi
grep -q 'objectName: "camperV2LeftEdgeSwipe"' "$stage/Victron/VenusOS/pages/camper/v2/CamperV2PanelHost.qml"
grep -q 'objectName: "camperV2RightEdgeSwipe"' "$stage/Victron/VenusOS/pages/camper/v2/CamperV2PanelHost.qml"
grep -q 'width: root.edgeWidth' "$stage/Victron/VenusOS/pages/camper/v2/CamperV2PanelHost.qml"
grep -q '/State/Weather' "$stage/Victron/VenusOS/data/camper/CamperWeatherAdapter.qml"
grep -q 'maxHourlyPrecipProbabilityPct' "$stage/Victron/VenusOS/pages/camper/v2/CamperV2WeatherPanel.qml"
grep -q 'adapter.setQuickAccessIds' "$stage/Victron/VenusOS/pages/camper/v2/CamperV2QuickPanel.qml"
grep -q 'openVictronSettings' "$stage/Victron/VenusOS/pages/camper/CamperShell.qml"
